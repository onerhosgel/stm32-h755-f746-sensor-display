...
/* H755 USART1 TX -> F746 USART6 RX (PC7) */
volatile uint32_t g_uartByteCount = 0U;
volatile uint32_t g_sensorPacketCount = 0U;

volatile int32_t g_temperature10 = 0;
volatile int32_t g_humidity10 = 0;
volatile int32_t g_pressure10 = 0;
volatile uint16_t g_red = 0U;
volatile uint16_t g_green = 0U;
volatile uint16_t g_blue = 0U;
volatile uint16_t g_clear = 0U;

/* UART packet parser working variables */
static volatile uint8_t g_packetActive = 0U;
static volatile uint8_t g_fieldIndex = 0U;
static volatile int32_t g_currentValue = 0;
static volatile int32_t g_fieldValues[7] = {0};
static volatile int8_t g_currentSign = 1;
/* USER CODE END PV */
...
static void USART6_RX_RegisterInit(void);
/* USER CODE END PFP */
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* PC7 = USART6_RX, 115200 8N1. No HAL_UART/CubeMX required. */
  USART6_RX_RegisterInit();
  /* USER CODE BEGIN 2 */
  /* USER CODE END 2 */
  /* Init scheduler */
  osKernelInitialize();
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOC, RENDER_TIME_Pin, GPIO_PIN_RESET);
  /*Configure GPIO pin Output Level */
...
/*
 * USART6 RX - direct register setup.
 * F746 PC7 (D0) = USART6_RX, AF8, 115200 8N1.
 * No HAL_UART module and no CubeMX regeneration needed.
 */
static void USART6_RX_RegisterInit(void)
{
    uint32_t pclk2;
    ...
    GPIOC->AFR[0] |=  (8U << (7U * 4U));

    /* USART6 clock source = PCLK2 (DCKCFGR2 USART6SEL[1:0] = 00) */
    RCC->DCKCFGR2 &= ~(3U << 10U);
    ...
    USART6->BRR = (pclk2 + 57600U) / 115200U;

    /* Receiver + RXNE interrupt + USART enable */
    USART6->CR1 = USART_CR1_RE | USART_CR1_RXNEIE | USART_CR1_UE;

    NVIC_SetPriority(USART6_IRQn, 5U);
...
void USART6_IRQHandler(void)
{
    ...
    {
        uint8_t rxByte = (uint8_t)(USART6->RDR & 0xFFU);

        g_uartByteCount++;

        /* Yeni paket: $TEMP,HUM,PRESS,R,G,B,CLEAR* */
        if (rxByte == '$')
        {
            g_packetActive = 1U;
            g_fieldIndex = 0U;
        ...
        else if (g_packetActive != 0U)
        {
            if ((rxByte >= '0') && (rxByte <= '9'))
            {
                g_currentValue = (g_currentValue * 10) + (int32_t)(rxByte - '0');
            }
            else if ((rxByte == '-') && (g_currentValue == 0))
            {
                g_currentSign = -1;
            }
            else if ((rxByte == ',') || (rxByte == '*'))
            {
                if (g_fieldIndex < 7U)
                {
                    g_fieldValues[g_fieldIndex] = g_currentValue * g_currentSign;
                    g_fieldIndex++;
                }
                ...
                if (rxByte == '*')
                {
                    if (g_fieldIndex == 7U)
                    {
                        g_temperature10 = g_fieldValues[0];
                        g_humidity10    = g_fieldValues[1];
                        g_pressure10    = g_fieldValues[2];
                        g_red           = (uint16_t)g_fieldValues[3];
                        g_green         = (uint16_t)g_fieldValues[4];
                        g_blue          = (uint16_t)g_fieldValues[5];
                        g_clear         = (uint16_t)g_fieldValues[6];
            ...
            {
                /* Beklenmeyen karakter -> paketi iptal et */
                g_packetActive = 0U;
                g_fieldIndex = 0U;
                g_currentValue = 0;
                g_currentSign = 1;
    ...
    if ((USART6->ISR & USART_ISR_ORE) != 0U)
    {
